...
export const challengeDraftDataSchema=z.object({
  ...
  restDaysPerWeek:z.coerce.number().int().min(0).max(6).default(2),
...
}).strict().superRefine((value,context)=>{if(value.numericMaximum<value.numericMinimum)context.addIssue({code:"custom",path:["numericMaximum"],message:"El máximo debe ser mayor o igual al mínimo"});});

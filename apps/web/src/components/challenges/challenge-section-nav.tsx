...
export function ChallengeSectionNav({invitationCount=0}:{invitationCount?:number}){
  ...
  function go(id:string){setActive(id);window.dispatchEvent(new CustomEvent("nova-gym:open-challenge-section",{detail:{id}}));window.requestAnimationFrame(()=>document.getElementById(id)?.scrollIntoView({behavior:"smooth",block:"start"}));}

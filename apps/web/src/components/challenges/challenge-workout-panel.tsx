...
export function ChallengeWorkoutPanel({
  ...
  return <>
    ...
    {mounted && open ? createPortal(<div className="fixed inset-0 z-[65] flex items-end justify-center bg-black/85 backdrop-blur-xl sm:items-center sm:p-5" onMouseDown={(event) => { if (event.target === event.currentTarget) closePanel(); }}>
      <section role="dialog" aria-modal="true" aria-label={`Entrenamientos de ${challengeName}`} className="flex max-h-[96dvh] w-full max-w-4xl flex-col overflow-hidden rounded-t-[30px] border border-slate-700 bg-[#070c16] shadow-2xl sm:rounded-[30px]">
        <header className="shrink-0 border-b border-slate-800 bg-slate-900/95 p-5 backdrop-blur-xl sm:p-6"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-black tracking-[.18em] text-lime-300">CENTRO DE COMPETENCIA</p><h2 className="mt-1 text-2xl font-black sm:text-3xl">Entrenamientos del reto</h2><p className="mt-1 text-sm muted">{challengeName} · evidencia, votos y lugares verificados</p></div><button type="button" onClick={closePanel} aria-label="Cerrar entrenamientos" className="inline-flex shrink-0 items-center gap-2 rounded-full border border-slate-700 bg-slate-950 px-3 py-2.5 text-xs font-black text-slate-200"><span className="hidden sm:inline">Volver a mis retos</span><X size={20}/></button></div>

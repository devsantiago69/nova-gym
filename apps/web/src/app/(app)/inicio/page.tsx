...
function currentStreak(dates: Date[]) {
  ...
  const elapsed = Math.round(
    (todayUtc - Date.parse(`${uniqueDays[0]}T00:00:00Z`)) / 86_400_000,
  );
  ...
  for (let index = 1; index < uniqueDays.length; index += 1) {
    if (
      Math.round(
        (Date.parse(`${uniqueDays[index - 1]}T00:00:00Z`) -
          Date.parse(`${uniqueDays[index]}T00:00:00Z`)) /
          86_400_000,
      ) !== 1
    )
      break;
...
export default async function Dashboard() {
  ...
  const profile = await prisma.userProfile.findUnique({
    where: { userId },
    select: { firstName: true, timezone: true, locale: true, localeAuto: true },
  });
  ...
  const [
    ...
    prisma.pointLedger.aggregate({
      where: { userId, logicalDate: { gte: monthStart } },
      _sum: { amount: true },
    }),
    prisma.attendance.findMany({
      where: { userId, status: "COMPLETED", invalidatedAt: null },
      select: { localDate: true },
      orderBy: { localDate: "desc" },
    }),
    prisma.attendance.findFirst({
      where: { userId, localDate: today, invalidatedAt: null },
        ...
        requester: {
          include: {
            ...
            attendances: {
              ...
              select: { finishedAt: true, durationMinutes: true },
              take: 1,
            },
          },
        },
    ...
    loadSocialFeed(userId, 6),
  ]);

  const completedKeys = new Set(
    attendances.map(({ localDate }) => dayKey(localDate)),
  );
  ...
  const friends = friendships
    ...
    .sort(
      (left, right) =>
        Number(Boolean(right.attendance)) - Number(Boolean(left.attendance)),
    );
  ...
  return (
    <div className="relative space-y-7 pb-8">
      ...
      <header className="flex flex-wrap items-end justify-between gap-4 pt-2">
        ...
        <Link
          href="/asistencia"
          className="btn inline-flex items-center gap-2 px-5 py-3"
        >
          <Dumbbell size={18} />
          {completedToday
      ...
      <section className="relative isolate overflow-hidden rounded-[34px] border border-white/[.09] bg-[radial-gradient(circle_at_90%_0%,rgba(163,230,53,.18),transparent_34%),radial-gradient(circle_at_5%_100%,rgba(34,211,238,.1),transparent_35%),rgba(10,18,32,.78)] p-6 shadow-[0_30px_90px_rgba(0,0,0,.24)] backdrop-blur-2xl sm:p-8">
        ...
        <div className="grid gap-7 lg:grid-cols-[1fr_1.2fr] lg:items-center">
          <div>
            <span
              className={`inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-black ${completedToday ? "bg-lime-400/15 text-lime-300" : activeAttendance ? "bg-orange-400/15 text-orange-300" : "bg-slate-800 text-slate-300"}`}
            >
              {completedToday ? (
          ...
          <div className="grid grid-cols-7 gap-2">
            {week.map(({ key, date }) => {
              ...
              return (
                <div
                  key={key}
                  className={`rounded-2xl border px-1 py-3 text-center ${done ? "border-lime-400/40 bg-lime-400/15" : isToday ? "border-cyan-400/40 bg-cyan-400/10" : "border-white/5 bg-black/20"}`}
                >
                  ...
                  <span
                    className={`mt-2 grid h-8 place-items-center text-sm font-black ${done ? "mx-auto w-8 rounded-full bg-lime-400 text-slate-950" : "text-slate-300"}`}
                  >
                    {done ? <CheckCircle2 size={17} /> : date.getUTCDate()}
                  </span>
                </div>
              );
            })}
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        {stats.map(([label, value, Icon, color]) => (
          ...
          >
            ...
            <strong className="mt-4 block text-2xl sm:text-3xl">{value}</strong>
            <span className="text-xs muted sm:text-sm">{label}</span>
          </article>
        ))}
      </div>
          ...
          <div className="border-t border-white/[.06] p-4 sm:p-5">
            <div className="mb-5 rounded-2xl bg-slate-950/70 p-4">
              ...
              <div className="mt-3 h-2.5 overflow-hidden rounded-full bg-slate-800">
                <div
                  ...
                  style={{
                    width: `${friends.length ? Math.round((friendsDone / friends.length) * 100) : 0}%`,
                  }}
                />
              </div>
            </div>
            {friends.length === 0 ? (
              <div className="py-8 text-center">
                ...
                <p className="mt-3 font-bold">Tu equipo empieza aquí</p>
                <Link
                  href="/comunidad"
                  className="mt-2 inline-block text-sm font-bold text-lime-300"
                >
                  Encontrar amigos
                </Link>
              </div>
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                {friends.slice(0, 6).map((friend) => (
                  <article
                    key={friend.id}
                    className="flex items-center gap-3 rounded-2xl border border-slate-800 bg-slate-950/45 p-3"
                  >
                    <span
                      className={`grid h-11 w-11 shrink-0 place-items-center rounded-2xl font-black ${friend.attendance ? "bg-lime-400 text-slate-950" : "bg-slate-800 text-slate-300"}`}
                    >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <span
                        className={`mt-0.5 inline-flex items-center gap-1 text-xs ${friend.attendance ? "text-lime-300" : "text-slate-500"}`}
                      >
                        {friend.attendance ? (
                          <>
                    ...
                    {friend.attendance?.durationMinutes && (
                      <small className="text-slate-500">
                        {friend.attendance.durationMinutes} min
                      </small>
                    )}
                  </article>
                ))}
          ...
          <div className="space-y-3 border-t border-white/[.06] p-4 sm:p-5">
            {challenges.length === 0 ? (
              <div className="py-8 text-center">
                ...
                <p className="mt-3 font-bold">Activa tu primera meta</p>
                <Link
                  href="/retos"
                  className="mt-2 inline-flex items-center gap-1 text-sm font-bold text-lime-300"
                >
                  Explorar retos <ChevronRight size={15} />
                </Link>
              </div>
            ) : (
              challenges.map((challenge) => {
                const count = challenge.scoreEvents.length;
                const progress = Math.min(
                  100,
                  Math.round((count / challenge.targetValue) * 100),
                );
                return (
                  <Link
                    href="/retos"
                    key={challenge.id}
                    className="block rounded-2xl border border-slate-800 bg-slate-950/50 p-4 transition hover:border-orange-400/40"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <span className="text-xs muted">
                          {count} de {challenge.targetValue} asistencias
                        </span>
                      </div>
                      <span className="rounded-full bg-orange-400/10 px-2.5 py-1 text-xs font-black text-orange-300">
                        {progress}%
                      </span>
                    </div>
                    ...
                    <div className="mt-3 flex items-center justify-between text-xs">
                      <span className="inline-flex items-center gap-1 text-slate-400">
                        ...
                        {challenge.participants[0]?.score ?? 0} puntos
                      </span>
                      <span className="text-slate-500">
                        {Math.max(0, challenge.targetValue - count)} para la
                        meta
                      </span>
                    </div>
                  </Link>
      ...
      <section className="grid gap-3 sm:grid-cols-3">
        <Link
          href="/asistencia"
          className="group card flex items-center gap-4 p-5 transition hover:-translate-y-1 hover:border-lime-400/40"
        >
          <span className="grid h-12 w-12 place-items-center rounded-2xl bg-lime-400/10 text-lime-300">
            <Dumbbell />
          </span>
          <div className="flex-1">
            <strong>Registrar entrenamiento</strong>
            <p className="text-xs muted">Suma a tu racha diaria</p>
          </div>
          <ChevronRight className="text-slate-600 group-hover:text-lime-300" />
        </Link>
        <Link
          href="/retos"
          className="group card flex items-center gap-4 p-5 transition hover:-translate-y-1 hover:border-orange-400/40"
        >
          <span className="grid h-12 w-12 place-items-center rounded-2xl bg-orange-400/10 text-orange-300">
            <Flame />
          </span>
          <div className="flex-1">
            <strong>Ver historias del equipo</strong>
            <p className="text-xs muted">Valida esfuerzos privados</p>
          </div>
          <ChevronRight className="text-slate-600 group-hover:text-orange-300" />
        </Link>
        <Link
          href="/comunidad"
          className="group card flex items-center gap-4 p-5 transition hover:-translate-y-1 hover:border-cyan-400/40"
        >
          <span className="grid h-12 w-12 place-items-center rounded-2xl bg-cyan-400/10 text-cyan-300">
            <UsersRound />
          </span>
          <div className="flex-1">
            <strong>Ampliar mi equipo</strong>
            <p className="text-xs muted">Conecta y crea nuevos retos</p>
          </div>
          <ChevronRight className="text-slate-600 group-hover:text-cyan-300" />
        </Link>
      </section>
    </div>
  );

...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!storage.plan) return fail("PLAN_REQUIRED", "Necesitas un plan activo para guardar evidencias", 403);
    ...
    const key = `attendance/${session.user.id}/${attendance.id}/end.webp`; await putPrivateObject(key, image.body, image.mimeType);

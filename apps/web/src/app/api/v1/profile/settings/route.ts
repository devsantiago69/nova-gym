...
export async function PATCH(request: Request) {
  ...
  if (!parsed.success) return fail("VALIDATION_ERROR", parsed.error.issues[0]?.message ?? "Revisa tus datos", 422);
  try {
    ...
    const response = ok({ locale: resolvedLocale, localeAuto: updated.localeAuto, timezone: updated.timezone }, "Tu perfil quedó actualizado");
    response.cookies.set("nova_locale", resolvedLocale, { path: "/", sameSite: "lax", secure: process.env.NODE_ENV === "production", maxAge: 60 * 60 * 24 * 365 });
    return response;
  } catch (error) {
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") return fail("PROFILE_VALUE_IN_USE", "El usuario, correo o WhatsApp ya pertenece a otra cuenta", 409);
    console.error("profile.settings.update.failed", { userId: session.user.id, error });

...
export async function POST(request: Request) {
  ...
  if (recent >= 10)
    return fail(
      "REGISTER_RATE_LIMIT",
      "Se alcanzó el límite de registros. Intenta nuevamente en una hora.",
      429,
    );
  ...
  try {
      ...
      async (tx) => {
        ...
        await tx.auditLog.create({
          data: {
            ...
            newValues: { username: created.username, plan: plan.code },
          },
        });
        return created;
      },
    ...
    if (error instanceof Error && error.message === "FREE_PLAN_UNAVAILABLE")
      return fail(
        "PLAN_UNAVAILABLE",
